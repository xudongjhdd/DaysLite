...
- [x] 免费 App 协议状态有效；本次免费 App 上架不需要签署付费 App 协议。
- [ ] 已确认商店显示的个人开发者名称符合预期。
...
- [x] Primary Language、分类和版本信息已填写。
- [x] Support URL 可匿名访问。
- [x] Privacy Policy URL 可匿名访问。
- [x] 年龄分级问卷已按实际功能完成，结果为 4+（有区域例外）。
- [x] App Privacy 已按当前版本选择“不收集数据”并发布。
- [x] 内容版权、出口合规和其他必填问卷已由账号持有人确认。
...
- [x] 审核备注草稿已完成。
- [x] 开发阶段截图已保存用于功能证据。
- [x] 已按 Apple 提交时的官方尺寸生成英文最终截图。
- [x] 已按 Apple 提交时的官方尺寸生成简体中文最终截图。
- [x] 所有截图无个人信息、键盘、调试文字、裁切或错误状态。
- [x] 商店名称、描述和截图只展示当前构建已有功能。
...
- [x] 内部 TestFlight 真机验收通过（用户确认安装和验收完成）。
- [x] 未产生新构建；商店已选择最终构建 `1.0.0 (1)`。

## F. 审核与发布

- [x] 版本页已选择最终构建。
- [x] App Review 联系方式和审核备注已填写。
- [x] 已完成 Add for Review。
- [x] 账号持有人已最终核对并 Submit for Review。
- [x] 已记录当前审核状态“等待审核”、提交时间和提交 ID。
- [ ] 审核通过后已手动发布。
